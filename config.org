...
(map! :leader
      (:prefix ("t" . "Toggle")
       :desc "Frame maximize" "M" #'toggle-frame-maximized))
#+end_src
...
#+begin_src emacs-lisp :tangle (if (my/at-home-p) "yes" "no")
...
#+begin_src emacs-lisp
(after! vhdl-mode
  (let ((funcs '(ec-vhdl-init lsp! ec-vhdl-tool-start-server yas-minor-mode-on)))
    (mapc (lambda (f)
            (add-hook! vhdl-mode f))
          funcs)))
#+end_src
...
#+begin_src emacs-lisp :tangle (if (my/at-office-p) "yes" "no")
...
#+begin_src emacs-lisp :tangle (if (my/at-office-p) "packages.el" "no")
...
*** optiver-eml-plus
#+begin_src emacs-lisp :tangle (if (my/at-office-p) "packages.el" "no")
(package! optiver-eml-plus
  :recipe (:type git
           :repo "ssh://git@git:7999/~enzchi/optiver-eml-plus.git"))
#+end_src

#+begin_src emacs-lisp :tangle (if (my/at-office-p) "yes" "no")
(use-package! optiver-eml-plus
  :commands (optiver-eml-plus-dispatch optiver-eml-plus-rerun-previous-command))
#+end_src

*** optiver-boxes
#+begin_src emacs-lisp :tangle (if (my/at-office-p) "packages.el" "no")
(package! optiver-boxes
  :recipe (:type git
           :repo "ssh://git@git:7999/~enzchi/optiver-boxes.git"))
#+end_src

#+begin_src emacs-lisp :tangle (if (my/at-office-p) "yes" "no")
(use-package! optiver-boxes)
#+end_src
